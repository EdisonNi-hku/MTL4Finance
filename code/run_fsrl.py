...
from utils import set_seed, AttnTrainer, prepare_semantic_parsing_data, srl_id2tag, srl_unique_tags
...
if __name__ == '__main__':
    ...
    train_dataset, train_tags = prepare_semantic_parsing_data('train', tokenizer)
    val_dataset, val_tags = prepare_semantic_parsing_data('valid', tokenizer)
    test_dataset, test_tags = prepare_semantic_parsing_data('test', tokenizer)
    ...
    if args.test == 'no':
        if args.cont == 'no':
            ...
            if args.pal:
                ...
                model = BertForTokenClassificationAttnPal.from_pretrained(model_name, config=model_config, local_files_only=True, cache_dir='cache')
                if args.load_path != '':
                ...
                model_config = BertForTokenClassificationAttnConfig.from_pretrained(args.model,
                                                                                       ...
                                                                                       pool=args.pool,
                                                                                       num_labels=len(srl_unique_tags))
                model = BertForTokenClassificationAttn.from_pretrained(model_name, config=model_config, local_files_only=True, cache_dir='cache')
                if args.load_path != '':
                    model.bert.load_state_dict(bert_state_dict)
