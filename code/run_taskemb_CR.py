...
from models.modeling_task_embeddings import BertForSequenceClassification, BertTaskEmbeddingConfig
...
MODEL_CLASSES = {
    'bert': (BertTaskEmbeddingConfig, BertForSequenceClassification, BertTokenizerFast)
}
...
def compute_Fisher(args, model, input_mask, total_tokens):
    ...
    # cls output
    name = 'cls_output'
    score = base_model._get_model_outputs(
        key=name).grad if args.feature_type == 'grads' else base_model._get_model_outputs(
        key=name)  # batch_size x hidden_size
...
def main():
    ...
    if args.glue_task:
        ...
        if args.task_name == 'asa':
            train_dataset = prepare_aspect_sentiment_data(True, 'train', tokenizer)
            args.num_labels = 1
        elif args.task_name == 'fpb':
            train_dataset = prepare_FPB_data('train', tokenizer)
            args.num_labels = 3
        elif args.task_name == 'na':
            train_dataset = prepare_number_attachment_data(True, 'train', tokenizer)
            args.num_labels = 2
        elif args.task_name == 'nc':
            train_dataset = prepare_number_classification_data(True, 'train', tokenizer)
            args.num_labels = len(number_labels)
        else:
