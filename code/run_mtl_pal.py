...
from models.models import BertForSequenceClassificationAttnPal, \
    BertForSequenceClassificationAttnPalConfig, BertForTokenClassificationAttnPal, BertForTokenClassificationAttnPalConfig
...
def main():
    ...
    parser.add_argument('--freeze_type', type=int, default=0)
    ...
    model_config_dict = {
        "asa": BertForSequenceClassificationAttnPalConfig.from_pretrained(args.model, num_labels=1, layer_attn=args.layer_attn, soft=args.soft, pool=True, arch=args.pal_arch, tuning_size=args.tunning_size),
        "na": BertForSequenceClassificationAttnPalConfig.from_pretrained(args.model, num_labels=2, layer_attn=args.layer_attn, soft=args.soft, pool=True, arch=args.pal_arch, tuning_size=args.tunning_size),
        "fpb": BertForSequenceClassificationAttnPalConfig.from_pretrained(args.model, num_labels=3, layer_attn=args.layer_attn, soft=args.soft, pool=True, arch=args.pal_arch, tuning_size=args.tunning_size),
        "srl": BertForTokenClassificationAttnPalConfig.from_pretrained(args.model, num_labels=len(srl_unique_tags), layer_attn=args.layer_attn, soft=args.soft, pool=True, arch=args.pal_arch, tuning_size=args.tunning_size),
        "nc": BertForSequenceClassificationAttnPalConfig.from_pretrained(args.model, num_labels=len(number_labels), layer_attn=args.layer_attn, soft=args.soft, pool=True, arch=args.pal_arch, tuning_size=args.tunning_size),
        "cau": BertForTokenClassificationAttnPalConfig.from_pretrained(args.model, num_labels=len(cause_tags), layer_attn=args.layer_attn, soft=args.soft, pool=True, arch=args.pal_arch, tuning_size=args.tunning_size),
    }
    model_type_dict = filter_dict(tasks, model_type_dict)
    ...
    if args.load == 'no':
        ...
        if args.freeze:
            for i, _ in enumerate(multitask_model.encoder.encoder.layer):
                if args.freeze_type == 1 and i in [0, 1, 2, 3]:
                    continue
        ...
        test_config_dict = {
            ...
            'fpb': BertForSequenceClassificationAttnPalConfig.from_pretrained(args.model, num_labels=3, layer_attn=args.layer_attn, soft=args.soft, pool=True, arch=args.pal_arch, tuning_size=args.tunning_size),
            'srl': BertForTokenClassificationAttnPalConfig.from_pretrained(args.model, num_labels=len(srl_unique_tags), layer_attn=args.layer_attn, soft=args.soft, pool=True, arch=args.pal_arch, tuning_size=args.tunning_size),
            'nc': BertForTokenClassificationAttnPalConfig.from_pretrained(args.model, num_labels=len(number_labels), layer_attn=args.layer_attn, soft=args.soft, pool=True, arch=args.pal_arch, tuning_size=args.tunning_size),
            'cau': BertForTokenClassificationAttnPalConfig.from_pretrained(args.model, num_labels=5, layer_attn=args.layer_attn, soft=args.soft, pool=True, arch=args.pal_arch, tuning_size=args.tunning_size),
        }

        test_model_dict = {
            'asa': BertForSequenceClassificationAttnPal.from_pretrained(args.model, config=test_config_dict['asa']),
            'na': BertForSequenceClassificationAttnPal.from_pretrained(args.model, config=test_config_dict['na']),
            'fpb': BertForSequenceClassificationAttnPal.from_pretrained(args.model, config=test_config_dict['fpb']),
            'srl': BertForTokenClassificationAttnPal.from_pretrained(args.model, config=test_config_dict['srl']),
            'nc': BertForSequenceClassificationAttnPal.from_pretrained(args.model, config=test_config_dict['nc']),
            'cau': BertForTokenClassificationAttnPal.from_pretrained(args.model, config=test_config_dict['cau']),
        }
